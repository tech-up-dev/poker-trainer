...
const { data, error } = await prodSupabase.from('user_progress').select('*')
...
useEffect(() => {
  const {
    data: { subscription },
  } = supabaseProd.auth.onAuthStateChange(async (event, session) => {
    if (session) {
...
export function useHasAccess(key: string): boolean {
  ...
  return entitlements.some(
    (e) =>
      e.entitlement_key === key && (e.expires_at === null || new Date(e.expires_at) > new Date()),
  )
}
...
export async function getUserFromRequest(req: Request, supabase: SupabaseClient): Promise<string> {
...
```typescript
const ALLOWED_ORIGINS = ['https://yourapp.vercel.app', 'https://poker.yourdomain.com']
...
const {
  data: { publicUrl },
} = supabase.storage.from('avatars').getPublicUrl(`player-types/${filename}`)
```
