...
- **RailBird** — borrow the "Today's Spot" home-screen treatment and the prominent streak surface. *Not* the 6-max playable-hand layout.
...
*(Steve listed "19" twice — SPR and Continuation Betting. Both are seeded.)*
...
- Service worker caching strategy is designed *together with* the content publish flow so a "promote to production" always reaches members cleanly (no stale-content bug after publish).
