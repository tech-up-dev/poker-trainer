...
export function GlossaryEditor({ onPublishedContextChange }: GlossaryEditorProps): JSX.Element {
  ...
  useEffect(() => {
    const contentId =
      validationResult?.ok === true ? validationResult.data.term_id : null
    onPublishedContextChange({ contentId, contentType: contentId ? 'glossary' : null, refreshSignal: versionsRefresh })
  ...
  async function handlePromote(): Promise<void> {
    ...
    const { data, error } = await supabaseProd.functions.invoke('promote-to-prod', {
      body: { content_id: entry.term_id, content_type: 'glossary' },
    })
    if (error) {
  ...
  return (
    <section className="space-y-6">
      ...
      <div aria-live="polite" className="space-y-3">
        ...
        {saveStatus === 'saved' && validationResult?.ok === true ? (
          <p className="text-sm text-green-400">
            Saved to staging as {validationResult.data.term_id}
          </p>
        ) : null}
      ...
      {validatedTermId !== null ? (
        <VersionsPanel
          contentId={validatedTermId}
