...
export function ContentBody({
  ...
  return (
    <pre className="text-xs text-ink-2 bg-canvas border border-line rounded p-4 overflow-x-auto whitespace-pre-wrap break-words">
...
function QuestionCard({ question, index }: QuestionCardProps): JSX.Element {
  return (
    <article className="rounded border border-line bg-canvas/60 p-4 space-y-3">
      ...
      <p className="text-sm text-ink leading-relaxed">{question.prompt}</p>
          ...
          <li
            ...
            className={
              answer.is_correct
                ...
                : 'pl-3 border-l-2 border-line'
            }
          >
            <div className={answer.is_correct ? 'text-green-300 font-medium' : 'text-ink-2'}>
              {answer.is_correct ? '✓ ' : ''}
              {answer.text}
            </div>
            <div className="text-xs text-ink-3 leading-relaxed">{answer.explanation}</div>
          </li>
        ))}
      </ul>
      ...
      {question.glossary_terms !== undefined && question.glossary_terms.length > 0 ? (
        <div className="flex flex-wrap gap-1 text-xs">
          {question.glossary_terms.map((term) => (
            <span key={term} className="px-2 py-0.5 rounded bg-surface text-ink-2">
              {term}
            </span>
          ))}
        </div>
...
function TableStateView({ state }: { state: TableState }): JSX.Element {
  return (
    <details className="text-xs text-ink-2">
      ...
      <pre className="mt-2 p-2 rounded bg-canvas border border-line overflow-x-auto text-ink-2">
